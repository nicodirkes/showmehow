...
def log_likelihood(theta, model, data, noise_model):
    ...
    required_cols = {"shear_stress", "exposure_time", "Mean", "SD"}
...
if __name__ == "__main__":
    ...
    parser.add_argument('data_filename', help='CSV file with data')
    ...
    try:
        model = umbridge.HTTPModel("http://localhost:4343", args.model_name)
    ...
    try:
        ...
        # Save results
        np.savez("MCMC_stress_based.npz", 
                 trace=trace, samples=samples, lnprob=lnprob)
        print("Results saved to MCMC_stress_based.npz")
