...
def setup_output_directory():
    output_dir = Path("diag_results")
    output_dir.mkdir(exist_ok=True)
    print(f"Results will be saved to: {output_dir.absolute()}")
    return output_dir

def load_mcmc_results(filename="/Users/adam-b3fsuibehsxrtg3/showmehow/usecase6/mcmc/MCMC_stress_based.npz"):
...
def create_inference_data(samples, lnprob):
    ...
    idata = az.from_dict(posterior=posterior_dict, sample_stats=sample_stats_dict)
    print(f"Created InferenceData: {samples.shape[0]} chains, {samples.shape[1]} draws, {samples.shape[2]} parameters")
    return idata

def run_arviz_diagnostics(idata, output_dir):
    print("\n" + "="*50)
...
def create_arviz_plots(idata, output_dir):
    ...
    az.plot_posterior(idata, figsize=(12, 4), hdi_prob=0.94)
    ...
    az.plot_pair(idata, kind="scatter", marginals=True, figsize=(10, 10))
    plt.suptitle("Parameter Correlations", fontsize=14)
    plt.tight_layout()
    plt.savefig(output_dir / "parameter_correlations.png", dpi=150, bbox_inches='tight')
    plt.show()
    print("- Creating autocorrelation plots...")
    az.plot_autocorr(idata, figsize=(12, 4))
    ...
    az.plot_rank(idata, figsize=(12, 4))
    plt.suptitle("Rank Plots", fontsize=14)
    plt.tight_layout()
    plt.savefig(output_dir / "rank_plots.png", dpi=150, bbox_inches='tight')
    plt.show()
    try:
        print("- Creating energy plot...")
        az.plot_energy(idata, figsize=(8, 6))
...
def create_summary_table(idata, trace, output_dir):
...
def save_samples(trace, output_dir):
    param_names = ["theta_0", "theta_1", "theta_2"]
...
def main():
    try:
        ...
        trace, samples, lnprob = load_mcmc_results()
        idata = create_inference_data(samples, lnprob)
        rhat, ess_bulk, ess_tail, mcse = run_arviz_diagnostics(idata, output_dir)
        create_arviz_plots(idata, output_dir)
        create_summary_table(idata, trace, output_dir)
        save_samples(trace, output_dir)
...
- ESS > 400 (excellent) or > 100 (acceptable)
        ...
        print("Error: MCMC_stress_based.npz not found!")
        print("Make sure you've run the MCMC script first.")
    except Exception as e:
        print(f"Error: {e}")
        raise
